...
def actor(inp,scope):
	with tf.variable_scope(scope):
		...
		mod.fcLayer(20,activation=M.PARAM_LRELU)
		mod.fcLayer(20,activation=M.PARAM_LRELU)
...
def build_graph():
	...
	envholder2 = tf.placeholder(tf.float32,[None,2])
	...
	q_target = EPS*c_real + reward_holder

	c_loss = tf.reduce_mean(tf.square(c_eval2 - q_target))

	a_loss = -tf.reduce_mean(c_eval)

	train_c = tf.train.RMSPropOptimizer(0.02).minimize(c_loss,var_list=var_c1)
	train_a = tf.train.RMSPropOptimizer(0.01).minimize(a_loss,var_list=var_a1)

	assign_a = soft_assign(var_a2,var_a1,1)
	assign_c = soft_assign(var_c2,var_c1,1)

	assign_a0 = assign(var_a2,var_a1)
	assign_c0 = assign(var_c2,var_c1)

	return [envholder,envholder2,reward_holder,aholder],a_eval,[c_loss,a_loss],[train_c,train_a],[assign_c,assign_a],[assign_c0,assign_a0]
