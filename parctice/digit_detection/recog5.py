...
for level in contour_levels:
	...
	# This is the part to show every line found
	"""
	print(centroid_lines)
	if len(centroid_lines) != 0:
		for line in centroid_lines:
			rho, theta = line
			rho *= HOUGH_LINE_RHO_RESOLUTION
			theta = (theta*HOUGH_LINE_THETA_RESOLUTION) + HOUGH_LINE_MIN_THETA
			a = np.cos(theta)
			b = np.sin(theta)
			x0 = a*rho
