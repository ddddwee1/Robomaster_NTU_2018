...
class data_reader():
	...
	def get_conf_mtx(self,inp,scaling):
		...
		for i in range(len(inp)):
			x = inp[i][0]
			y = inp[i][1]
			col = int(np.floor(x/32*scaling))
			row = int(np.floor(y/32*scaling))
	...
	def get_bbox_mtx(self,inp,scaling):
		...
		for i in range(len(inp)):
			...
			bbox_res[row][col][0] = int((x - col*32/scaling - 16/scaling ))
			bbox_res[row][col][1] = int((y - row*32/scaling - 16/scaling ))
