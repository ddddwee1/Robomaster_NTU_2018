...
def train_RPN(imgholder,biasholder,confholder,loss_functions,train_s,RPNcb):
	MAXITER = 1000000
	BSIZE = 1	
	...
	with tf.Session() as sess:
		...
		for iteration in range(MAXITER):
			...
			index= 1-nscale	#bias0[0],bias1[0],bias2[0],bias3[0],conf0[0],conf1[0],conf2[0],conf3[0]
			...
			b0,b1,c0,c1= sess.run([RPNcb[0],RPNcb[1],RPNcb[2],RPNcb[3]],feed_dict=feeddict)
			if iteration%1==0:
				#print('Iter:',iteration,'\tloss_functions:',loss_f)
				#print(b,c)
				img = img_batch[0].astype(np.uint8)
				img1 = np.float32(img).copy()
				img1 = np.uint8(img1)
				draw(img1,c0[0],b0[0],4,wait=5)
				draw(img,c1[0],b1[0],1,wait=5)
...
train_RPN(RPNholders[0],RPNholders[1],RPNholders[2], RPNlosses[0], train_steps[0],RPNcb)
