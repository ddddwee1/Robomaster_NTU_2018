...
class reader():
	...
	def __init__(self,annot,height=270*2,width=480*2,scale_range=[0.05,1.2],lower_bound=2,upper_bound=4,index_multiplier=1):
		# set class params
		self.height = height
		self.width = width
		...
		#bar = progressbar.ProgressBar(max_value=1461)
		f = open(annot)
		counter = 0
		for i in f:
			i = i.strip().split('\t')
	...
	def show_img(self,img,coord):
		...
		for x,y,w,h in coord:
			...
			w = int(w)
			h = int(h)
			cv2.rectangle(imgbuff,(x,y),(x-w,y-h),(0,0,255),2)
	...
	def get_mtx(self,imgsize,coord):
		...
		for x,y,w,h in coord:
			...
			#print (index)
			grid_size = int((np.exp2(self.index_multiplier*index+self.lower_bound))*2)
...
#a = reader('annotation.txt')
#while True:
	#img,result_dict=a.get_img()
	#print (result_dict)
#	print (b[1])
