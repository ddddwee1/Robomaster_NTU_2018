...
def train_RPN(imgholder,biasholder,confholder,biaslosses,conflosses,train_s,RPNcb):
	...
	with tf.Session() as sess:
		...
		reader = data_reader.reader('drive/Colab/Multi_loss/training/annotationC.txt',height=540,width=960,scale_range=[0.0005,1.2],lower_bound=3,upper_bound=7,index_multiplier=2)
...
def train_veri(imgholder,RPNcb,croppedholder,veri_conf_holder,veri_conf_loss,veri_train_step,veri_conf,veri_accuracy):
	...
	with tf.Session() as sess:
		...
		M.loadSess(modpath='drive/Colab/veri_original_image/training/model_RPN/100000.ckpt',sess=sess,var_list=M.get_trainable_vars('mainModel'))
		saver = tf.train.Saver()
		reader = data_reader.reader('drive/Colab/Multi_loss/training/annotationC.txt',height=540,width=960,scale_range=[0.01,1.5],lower_bound=2,upper_bound=5,index_multiplier=1)
		print('Reading finish')
		for iteration in range(MAXITER):
			img, train_dic = reader.get_img()
			for k in train_dic:
