...
#		print('a:',a,'\tind:',ind,'\ti:',i,'\tj:',j,'\tconf:',c[i][j],'\txbias',int(b[i][j][0]),'\tybias',int(b[i][j][1]))
		# cropping:
		x = int(b[i][j][0])+j*32+16           #x-coord of btm right of a the object center wrt to original image(256x256)
		y = int(b[i][j][1])+i*32+16           #y-coord of btm right of a the object center wrt to original image(256x256)
