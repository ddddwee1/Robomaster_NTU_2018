...
class Display:
    def __init__(self, scale, arenaWidth, arenaHeight):
        self._width = int(arenaWidth/scale)
        self._height = int(arenaHeight/scale)
    ...
    def drawObstacle(self, obstacle):
        topleft, bottomright = obstacle.getVertex();
        obstacleRect = pygame.Rect(topleft[0], topleft[1], bottomright[0]-topleft[0], bottomright[1]-topleft[1])
        pygame.draw.rect(self._screen, (0,0,0), obstacleRect)
    ...
    def drawAllObstacles(self, obstacleList):
        allObstacles = obstacleList.getList()
        if allObstacles == 0:
            pass
        else:
