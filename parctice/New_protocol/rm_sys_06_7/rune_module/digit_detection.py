...
def get_digits(image,bigbuff=False):
	...
	for contour in contours:
		...
		if cv2.contourArea(contour)<60 or cv2.contourArea(contour)>600:
			continue
	...
	for x,y in digits_7seg_rect:
		...
		img_sevseg=cv2.cvtColor(img_sevseg, cv2.COLOR_BGR2HLS)
		#cv2.imshow('bbccc',img_sevseg)
		img_sevseg_red = img_sevseg[:,:,1].copy()
		#cv2.imshow('bbccc',img_sevseg_red)
