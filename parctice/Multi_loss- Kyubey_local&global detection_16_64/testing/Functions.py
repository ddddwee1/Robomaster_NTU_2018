...
def crop_original(img,bias_mtx,conf_mtx,b,c,scale):
	"""
		...
		for r in range(row):
			for c in range(col):
				if conf_mtx[r][c][0] == 1:
					...
					#print("r:", r, "\tc:", c, "\t", inp2)
					inp2[0] = inp2[0]+c*32/scale+16/scale
					inp2[1] = inp2[1]+r*32/scale+16/scale
					break
		inp2[2] = inp2[2]*2
		inp2[3] = inp2[3]*2
...
def crop_original_test(img,b,c,scale):
	"""
	Input: Original Image, Confidence matrix and Bias matrix
	Output: Array of 5 cropped images(size 32x32)
	Description: Select the top 5 regions with highest confidence and return cropped images of those regions  
...
def crop_original_test_c(img,b,c,scale):
	"""
	Input: Original Image, Confidence matrix and Bias matrix
	Output: Array of 5 cropped images(size 32x32)
	Description: Select the top 5 regions with highest confidence and return cropped images of those regions  
...
def dual_crop_original_test(img,b0,b2,c0,c2):

	croppedImages = []
	inds = []

	croppedImages0,inds0= crop_original_test(img,b0,c0,4)
	croppedImages.append(croppedImages0[0])
	inds.append(inds0[0])

	croppedImages2,inds2 = crop_original_test(img,b2,c2,1)
	croppedImages.append(croppedImages2[0])
	inds.append(inds2[0])
